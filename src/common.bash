...
export CI_CONTAINER_NAME="ci_job_build_$GITHUB_RUN_ID"
export CI_REGISTRY_TAG="$GITHUB_SHA"

if [[ "$GITHUB_JOB" == "review" ]]; then
  export STAGE="$CI_ENVIRONMENT_SLUG"
else
  export STAGE="$GITHUB_JOB"
fi

if [[ "$GITHUB_JOB" == "canary" ]]; then
  export STAGE="production"
  export GITHUB_JOB="production"
...
get_secrets_for_creation() {
  ...
  if env | grep -i -e "^$GITHUB_JOB"_ > /dev/null; then
    IFS=$'\n'
    STAGE_SECRETS=$(env | grep -i -e "^$GITHUB_JOB")
    for i in $STAGE_SECRETS; do
      fullkey=$(echo $i | cut -d'=' -f1)
      stripped=$(echo $i | cut -d'_' -f2-)
      key=$(echo $stripped | cut -d'=' -f1)
...
get_deploy_events() {
  if env | grep -i -e '^NEWRELIC_' > /dev/null; then
    ...
    for i in $NEWRELIC; do
        ...
        if echo "$i" | grep -i -e "$GITHUB_JOB" | grep -i -e "APP_ID" > /dev/null; then
  ...
  if env | grep -i -e '^SLACK' > /dev/null; then
    ...
    for i in $SLACK; do
        if echo "$i" | grep -i -e "$GITHUB_JOB" | grep -i -e "WEBHOOK" > /dev/null; then
  ...
  if env | grep -i -e '^INSTANA' > /dev/null; then
    ...
    for i in $INSTANA; do
        if echo "$i" | grep -i -e "$GITHUB_JOB" | grep -i -e "API_TOKEN" > /dev/null; then
